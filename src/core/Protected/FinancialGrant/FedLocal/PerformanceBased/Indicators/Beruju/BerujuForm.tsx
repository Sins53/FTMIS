...
import { BerujuInitialValueProps, BerujuValidationSchema } from './berujuSchema';
import { base } from '@/theme/colors';

interface BerujuFormProps {
  toggle: () => void;
  isOpen?: boolean;
  formData: BerujuInitialValueProps;
}
function BerujuForm(props: BerujuFormProps) {
  ...
  const { values, errors, handleChange, handleSubmit, touched, handleBlur, resetForm } = useFormik({
    ...
    onSubmit: (values: BerujuInitialValueProps, { resetForm }) => {
        ...
        <form
          ...
          <ModalBody>
            ...
            <Box className="row">
              <Box className="col-6">
                <Box className="mb-2">
                  <Label htmlFor="audited_amount">Audited Amount</Label>
                  <Input
                    value={values.audited_amount}
                    type="number"
                    onChange={handleChange}
                    onBlur={handleBlur}
                    name="audited_amount"
                  />
                  <FormikValidationError name="audited_amount" errors={errors} touched={touched} />
                </Box>
              </Box>
              <Box className="col-6">
                <Box className="mb-2">
                  <Label htmlFor="irregularities_amount">Irregularities Amount</Label>
                  <Input
                    value={values.irregularities_amount}
                    onChange={handleChange}
                    onBlur={handleBlur}
                    type="number"
                    name="irregularities_amount"
                  />
