...
import { PeriodicPlanInitialValueProps, PeriodicPlanValidationSchema } from './periodicPlanSchema';
import { base } from '@/theme/colors';

interface PeriodicPlanFormProps {
  toggle: () => void;
  isOpen?: boolean;
  formData: PeriodicPlanInitialValueProps;
}
function PeriodicPlanForm(props: PeriodicPlanFormProps) {
  ...
  const { values, errors, handleChange, handleSubmit, touched, handleBlur, resetForm } = useFormik({
    ...
    validationSchema: PeriodicPlanValidationSchema,
    onSubmit: (values: PeriodicPlanInitialValueProps, { resetForm }) => {
...
export default PeriodicPlanForm;
