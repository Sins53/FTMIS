...
import { SutraPortalInitialValueProps, SutraPortalValidationSchema } from './sutraPortalSchema';
import { base } from '@/theme/colors';

interface SutraPortalFormProps {
  toggle: () => void;
  isOpen?: boolean;
  formData: SutraPortalInitialValueProps;
}
function SutraPortalForm(props: SutraPortalFormProps) {
  ...
  const { values, errors, handleChange, handleSubmit, touched, handleBlur, resetForm } = useFormik({
    ...
    validationSchema: SutraPortalValidationSchema,
    onSubmit: (values: SutraPortalInitialValueProps, { resetForm }) => {
...
export default SutraPortalForm;
