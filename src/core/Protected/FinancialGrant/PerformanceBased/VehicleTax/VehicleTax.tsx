...
import {
  VehicleTaxDetailsData,
  VehicleTaxInitialValue,
  VehicleTaxInitialValueProps,
  VehicleTaxValidationSchema
} from './vehicleSchema';
import { useVehicleTaxDataCreator, useVehicleTaxDetailsData } from './vehicleTaxQueries';

function VehicleTax() {
  const { t } = useTranslation();
  const { data: provinceData } = useProvinceData({});
  const { data: vehicleTaxDetailsData, isLoading } = useVehicleTaxDetailsData({});
  const { mutate: vehicleTaxMutate } = useVehicleTaxDataCreator();

  const [isEdit, setIsEdit] = useState(false);
  const [formData, setFormData] = useState(VehicleTaxInitialValue);

  useEffect(() => {
    const arr: VehicleTaxInitialValueProps = { data: [] };
    provinceData &&
  ...
  const {
    ...
    validationSchema: VehicleTaxValidationSchema,
    onSubmit: (values: any, { resetForm }) => {
      const requestData = { ...values };
      vehicleTaxMutate(requestData, {
        onSuccess: () => {
          resetForm();
          setIsEdit(false);
        }
  ...
  const columns = useMemo(() => {
    const column = [
      {
        Header: 'Province',
        Cell: ({ row }: Cell<VehicleTaxDetailsData>) => {
          return (
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          {vehicleTaxDetailsData && vehicleTaxDetailsData?.length > 0 && !isEdit ? (
            <>
              <FlexBox align="center" justify="space-between" className="p-3">
                <Text variant="h6" color={base.primary} typeface="semiBold">
                  Monthly Distribution of Vehicle Tax
                </Text>
                <Button
                  className="btn btn-outline-primary"
                  onClick={() => {
                    setFormData({
                      data: vehicleTaxDetailsData?.map((item) => {
                        return {
              ...
              <Box className="px-3 flex-grow-1">
                <Table data={vehicleTaxDetailsData || []} columns={columns} loading={isLoading} />
              </Box>
            </>
          ) : (
